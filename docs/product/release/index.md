...
VMware provides a number reference designs for deploying Tanzu. The reference designs are available at [VMware Tanzu Reference Architecture Documentation](https://docs.vmware.com/en/VMware-Tanzu-Reference-Architecture/index.html). 
...
2. Log in to vSphere server/vSphere Client. 
3. Go to **Actions > Deploy OVF Template**. 
   1. Select the **Local file** option to upload the Service Installer OVA. 
   2. Provide the required computer resources and storage details.
   3. For **Applicance Network**, select the management port group.
   4. Specify the NTP server and the root password for the VM.
